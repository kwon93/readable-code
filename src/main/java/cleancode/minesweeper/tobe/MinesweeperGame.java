...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlantFlag(userActionInput)) {
            BOARD[selectedRowIndex][ selectedColIndex] = Cell.of("⚑");
            checkIfGameIsOver();
            return;
        }
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD)
                ...
                .noneMatch(cell -> cell.equalsSign("□"));
    }
    ...
    private static int convertColFrom(char cellInputCol) {
        switch (cellInputCol) {
            ...
            case 'j':
                return 9;
            default:
                throw new IllegalArgumentException("잘못된 입력이에용");
        }
    }
    ...
    private static void open(int row, int col) {
        ...
        if (LAND_MINE_COUNTS[row][col] != 0) {
            BOARD[row][col] = Cell.of(String.valueOf(LAND_MINE_COUNTS[row][col]));
            return;
        } else {
            BOARD[row][col] = Cell.of("■");
        }
